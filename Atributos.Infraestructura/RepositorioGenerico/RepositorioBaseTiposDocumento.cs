...
namespace Atributos.Infraestructura.RepositorioGenerico
{
    public class RepositorioBaseTiposDocumento<T> : IRepositorioBaseTiposDocumento<T> where T : EntidadBase
    {
        ...
        private TiposDocumentoDBContext GetContext()
        {
            return _serviceProvider.GetService<TiposDocumentoDBContext>();
        }
        ...
        public async Task<List<T>> BuscarPorAtributo(int ValueAttribute,string Attribute)
        {
            var _context = GetContext();
            var entitySet = _context.Set<T>();
            var res = await entitySet.Where(v => EF.Property<int>(v, Attribute) == ValueAttribute).ToListAsync();
            await _context.DisposeAsync();
            return res;
        }
